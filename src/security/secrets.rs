...
// in `~/.labaclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.labaclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(labaclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: labaclaw_dir.join(".secret_key"),
            enabled,
        }
    }
