...
impl TelegramChannel {
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        let labaclaw_dir = home.join(".labaclaw");
        ...
        config.workspace_dir = labaclaw_dir.join("workspace");
        Ok(config)
    }
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        ...
        let Some(telegram) = config.channels_config.telegram.as_mut() else {
            anyhow::bail!(
                ...
                or run `labaclaw onboard --channels-only` to configure interactively"
            );
        };
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to LabaClaw now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
