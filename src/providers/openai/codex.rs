...
fn parse_assistant_native_content(raw: &str) -> Option<String> {
    let payload = parse_assistant_tool_calls_payload(raw)?;
    first_nonempty(payload.content.as_deref())
        .or_else(|| first_nonempty(payload.reasoning_content.as_deref()))
}

fn parse_tool_result_content(raw: &str) -> Option<String> {
    let payload = parse_tool_result_payload(raw)?;
    let call_id = first_nonempty(payload.tool_call_id.as_deref());
    let rendered_content = first_nonempty(payload.content.as_deref())?;
        ...
        for line in data_lines {
            ...
            if let Ok(event) = serde_json::from_str::<Value>(line) {
                ...
                anyhow::bail!(
                    ...
                    crate::providers::sanitize_api_error(line)
                );
            }
        }
        if !parsed_any {
            anyhow::bail!(
                ...
                crate::providers::sanitize_api_error(trimmed)
            );
        }
