...
const SERVICE_LABEL: &str = "com.labaclaw.daemon";
const WINDOWS_TASK_NAME: &str = "LabaClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "labaclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["labaclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "labaclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["labaclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "labaclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["labaclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["labaclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/labaclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/labaclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "labaclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove labaclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/labaclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: labaclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "labaclaw.service"]));
...
/// Check if the labaclaw user exists and has expected properties.
...
fn check_labaclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "labaclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser labaclaw && delgroup labaclaw",
            "addgroup -S labaclaw && adduser -S -s /sbin/nologin -H -D -G labaclaw labaclaw",
        )
    } else {
        ("userdel labaclaw", "useradd -r -s /sbin/nologin labaclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'labaclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'labaclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/labaclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: labaclaw user has home directory '{}' (expected /var/lib/labaclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_labaclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "labaclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_labaclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "labaclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "labaclaw"])
                .output()
                .context("Failed to create labaclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create labaclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: labaclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create labaclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create labaclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "labaclaw"])
            .output()
            .context("Failed to create labaclaw user")?;
    ...
    println!("✅ Created system user: labaclaw");
    Ok(())
}

/// Change ownership of a path to labaclaw:labaclaw
#[cfg(unix)]
fn chown_to_labaclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["labaclaw:labaclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to labaclaw:labaclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_labaclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_labaclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "labaclaw:labaclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to labaclaw:labaclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_labaclaw(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".labaclaw"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'labaclaw' cannot write {} ({details}). \
             Re-run `sudo labaclaw service install` and ensure ownership is labaclaw:labaclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/labaclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="labaclaw:labaclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/labaclaw/access.log"
error_log="/var/log/labaclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/labaclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo labaclaw service install"
        );
    }

    ensure_labaclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/labaclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/labaclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_labaclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by labaclaw:labaclaw)",
    ...
    chown_recursive_to_labaclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/labaclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "labaclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/labaclaw");
    println!("   Config path: /etc/labaclaw/config.toml");
    println!("   Start with: sudo labaclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("labaclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/labaclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "LabaClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/labaclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/labaclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"labaclaw\""));
        assert!(script.contains("description=\"LabaClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/labaclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/labaclaw daemon\""));
        assert!(!script.contains("env LABACLAW_CONFIG_DIR"));
        assert!(!script.contains("env LABACLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"labaclaw:labaclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/labaclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/labaclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/labaclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/labaclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/labaclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/labaclaw"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/labaclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/labaclaw/workspace'".to_string(),
                "labaclaw".to_string()
            ]
        );
    }
