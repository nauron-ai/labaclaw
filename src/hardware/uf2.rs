...
//!    `~/.labaclaw/firmware/pico/` if they aren't there yet.
...
/// LabaClaw serial protocol handler — written to the Pico after MicroPython boots.
...
/// Ensure `~/.labaclaw/firmware/pico/` exists and contains the bundled assets.
///
        ...
        if PICO_UF2.len() < 8 || PICO_UF2[..4] != UF2_MAGIC1 {
            bail!(
                "Bundled UF2 is a placeholder — download the real MicroPython UF2 from \
                 ...
                 src/firmware/pico/zeroclaw-pico.uf2, then rebuild LabaClaw."
            );
        }
...
pub async fn flash_uf2(mount_point: &Path, firmware_dir: &Path) -> Result<()> {
    ...
    if data.len() < 8 || data[..4] != UF2_MAGIC1 {
        bail!(
            "UF2 at {} does not look like a valid UF2 file (magic mismatch). \
             ...
             the existing file so LabaClaw can re-extract it.",
            uf2_src.display()
        );
    }
    ...
    bail!(
        "All copy methods failed. Run this command manually, then restart LabaClaw:\n\
